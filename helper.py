...
class EquationHelper:
    ...
    def apply_style(mob, style, scene):
        target_color = style.get("color", None)
        target_size = style.get("font_size", None)
    
        anims = []
    
        if target_color is not None:
            anims.append(mob.animate.set_color(target_color))
    
        if target_size is not None:
            # Font size change = scale trick in Manim
            scale_factor = target_size / getattr(mob, "font_size", target_size)
            anims.append(mob.animate.scale(scale_factor))
